...
class MyPipeline {
  ...
  var pipeline:Pipeline=null
   ...
   def  createClassifier(): DecisionTreeClassifier ={
          val classifier = new DecisionTreeClassifier() // 决策树分类器
          classifier.setLabelCol("label_index") // 设置结果标签列
        .setFeaturesCol("feature_index") // 设置特征标签列
        .setPredictionCol("prediction_col")// 设置预测表的预测列名称
